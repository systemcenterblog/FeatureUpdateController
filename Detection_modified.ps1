...
Process {
    ...
    if ($ScriptOperationPrerequisites -eq $true) {
        try {
            ...
            $GitHubBaseUrl = "https://raw.githubusercontent.com/systemcenterblog/FeatureUpdateController/main/"
    
            try {
                # Download the latest version manifest file from storage account to the temporary download destination
                $ManifestUrl = "$GitHubBaseUrl$ManifestFileName"
Invoke-WebRequest -Uri $ManifestUrl -OutFile $ManifestFilePath -UseBasicParsing -ErrorAction Stop
                                            ...
                                            if ($SetupConfigScriptFile.Type -eq "POSTOOBE") {
                                                ...
                                                if ($SetupConfigScriptFileName -ne $null) {
                                                    ...
                                                    try {
                                                        ...
                                                        $SetupCompleteUrl = "$GitHubBaseUrl$SetupConfigScriptFileName"
$SetupCompletePs1FilePath = Join-Path -Path $ProgramDataFeatureUpdateControllerRootPath -ChildPath $SetupConfigScriptFileName
Invoke-WebRequest -Uri $SetupCompleteUrl -OutFile $SetupCompletePs1FilePath -UseBasicParsing -ErrorAction Stop
                                                    }
                                                    catch [System.Exception] {
                                            ...
                                            if ($SetupConfigScriptFile.Type -eq "PostRollback") {
                                                ...
                                                if ($SetupConfigScriptFileName -ne $null) {
                                                    ...
                                                    try {
                                                        ...
                                                        $SetupRollbackUrl = "$GitHubBaseUrl$SetupConfigScriptFileName"
$SetupRollbackPs1FilePath = Join-Path -Path $ProgramDataFeatureUpdateControllerRootPath -ChildPath $SetupConfigScriptFileName
Invoke-WebRequest -Uri $SetupRollbackUrl -OutFile $SetupRollbackPs1FilePath -UseBasicParsing -ErrorAction Stop
                                                    }
                                                    catch [System.Exception] {
                                                        $ErrorMessage = "Failed to download $($SetupConfigScriptFileName) file. Error message: $($_.Exception.Message)"
                                                        Write-LogEntry -Value "- $($ErrorMessage)" -Severity 3
                                        ...
                                        foreach ($CustomAction in $ManifestContent.CustomActions) {
                                            ...
                                            if (Test-Path -Path $CustomActionFolderPath) {
                                                ...
                                                try {
                                                    ...
                                                    $CustomActionScriptUrl = "$GitHubBaseUrl$CustomAction.ScriptFile"
Invoke-WebRequest -Uri $CustomActionScriptUrl -OutFile $CustomActionScriptFilePath -UseBasicParsing -ErrorAction Stop
                                                }
                                                catch [System.Exception] {
                                                    ...
                                                    if (-not(Test-Path -Path $ModuleRegistryKey)) {
                                                        ...
                                                        try {
                                                            ...
                                                            try {
                                                                ...
                                                                $ModuleScriptUrl = "$GitHubBaseUrl$ScriptModule.ScriptFile"
Invoke-WebRequest -Uri $ModuleScriptUrl -OutFile $ModuleScriptFilePath -UseBasicParsing -ErrorAction Stop
                                                            }
                                                            catch [System.Exception] {
